...
#property version   "28.19.SURGICAL"  // V26.0 MATH-FIRST ENGINE: Pure Math Signal Generation + Full V25 Fixes
#property strict
...
input bool InpAlphaExpand = true;                // Enable V24 Alpha Expansion (false=V23 mode, true=V24/V25/V26 mode)
input bool InpElasticScoring = true;             // Enable V25 Elastic Scoring (requires InpAlphaExpand=true)
input bool InpMathFirst = true;                  // Enable V26 Math-First Pure Math Signals (requires InpAlphaExpand=true)
...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| V28.19 PATCH: FIXED HUBBLE TRAIL — No modify loops              |
//| V26 BUG: Ran every tick → 3,530 modifications PER TRADE         |
//| FIX: Static timer — max 1 modify per 30 seconds PER order       |
//+------------------------------------------------------------------+
void ManageSiliconX_HubbleTrail()
{
    if (!InpSX_EnablePendingTrail) return; // Master switch check
    
    // V28.19 FIX: Throttle pending trail to once per 30 seconds
    static datetime lastTrailModify = 0;
    if(TimeCurrent() - lastTrailModify < 30) return;
    ...
    if (buyStopCount == 1 && sellStopCount == 1) {
        ...
        double newBuyStopLevel = NormalizeDouble(Bid - (InpSX_PendingTrailStartPips * _Point) , _Digits);
        ...
        if (newBuyStopLevel < buyStopPrice) {
            if (!OrderModify(buyStopTicket, newBuyStopLevel, 0.0, 0.0, 0, CLR_NONE)) {
                ...
                lastTrailModify = TimeCurrent(); // V28.19: Record modification time
        ...
        double newSellStopLevel = NormalizeDouble(Ask + (InpSX_PendingTrailStartPips * _Point) , _Digits);
